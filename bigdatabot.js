...
bot.on('/ownamount', (msg) => {
	...
	let groupid = msg.chat.id
        let sqlcmd = "SELECT COUNT(*) AS amount, truncate(avg(char_length( `text` )),2) AS AVG_Length FROM messagetable WHERE userid = " + hash(msg.from.id) + " AND `text` NOT LIKE '/%' AND groupid = " + groupid + ";";
	db.getConnection(function(err, connection) {
	        connection.query(sqlcmd, function(err, rows){
        	        if(err)throw err;
                	msg.reply.text("Your current  amount of your own msgs is: " + util.inspect(rows[0].amount,false,null) + " and the AVG_length is: " + rows[0].AVG_Length, { asReply: true });
...
bot.on('/topingroup', (msg) => {
        bot.sendAction(msg.chat.id, 'typing');
	let groupid = msg.chat.id
        let sqlcmd = "SELECT DISTINCT COUNT( `msgid` ) AS `Msgs`, `userid` AS `User`, truncate(avg(char_length( `text` )),2) AS AVG_Length FROM `db`.`messagetable` AS `messagetable` WHERE `text` NOT LIKE '/%' AND groupid = " + groupid + " GROUP BY `userid` ORDER BY `Msgs` DESC LIMIT 10;"
        db.getConnection(function(err, connection) {
		connection.query(sqlcmd, function(err, rows){
                	if(err) throw err;
